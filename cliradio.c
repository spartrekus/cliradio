...
{ 
      ...
      for(i=0; str[i]!='\0'; i++)
      {
        ...
        {   fdcolr++; }
        else if ( str[i] == coldell ) 
        {   fdcoll++; colnbr++; }
        else if ( str[i] == coldelr ) 
        {   fdcolr++;    }
        else if (str[i] != '\n' && str[i] != '\n') 
        {
          snprintf( charo, PATH_MAX , "%c", str[i] );
          if ( ( mygivencol == colnbr ) && ( fdcoll == colnbr )  && ( fdcolr == colnbr-1 ) )
...
void void_draw()
{
  ...
  int linepos = 0; 
   ...
   if ( fexist( filesource ) == 1 )
   {
    ...
    strncpy( str_selection, statusbar_cellcontent , PATH_MAX );
    ...
    while ( gameover == 0 ) 
    {
        ...
        switch( ch ) 
        {
           ...
           case 'r':  
              ...
              mvprintw( foxi, 5 , "%s (%d)", strgetcol( str_selection , foxi) , foxi ); foxi++;
              if ( strcmp( strgetcol( str_selection, 3 ), "" ) != 0 ) 
                  ncurses_runwith( " mplayer -playlist ", strgetcol( str_selection, 3 ) );
              else if ( strcmp( strgetcol( str_selection, 2 ), "" ) != 0 ) 
                  ncurses_runwith( " mplayer ", strgetcol( str_selection, 2 ) );
              break;
